...
(function (global) {
    ...
    init: function () {
      ...
      if (_.logged_in == "true") {
        ...
        [].forEach.call(boxarr, function (item) {
          ...
          item.querySelector(".comment-avatar-image").src = _.dom.querySelector(
  ...
  var iDisqus = function () {
    ...
    _.dom = d.getElementById(
      typeof arguments[0] == "string" ? arguments[0] : "comment"
    );
    _.opts.api =
    ...
    _.opts.slug = !!_.opts.slug
      ? _.opts.slug.replace(/[^A-Za-z0-9_-]+/g, "")
      : "";
    _.opts.desc =
      _.opts.desc ||
      (!!d.querySelector('[name="description"]')
        ? d.querySelector('[name="description"]').content
        : "");
  ...
  iDisqus.prototype.init = function () {
    ...
    _.dom.innerHTML =
      '<div class="comment loading" id="idisqus">\n' +
      ...
      (_.opts.mode != 1
        ? '<div class="comment-form-auth"><button class="comment-form-login" title="使用 Disqus 帐号登录"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 200 200"><path fill="#2E9FFF" d="M102.535 167.5c-16.518 0-31.621-6.036-43.298-16.021L30.5 155.405l11.102-27.401A67.658 67.658 0 0 1 35.564 100c0-37.277 29.984-67.5 66.971-67.5 36.984 0 66.965 30.223 66.965 67.5 0 37.284-29.98 67.5-66.965 67.5zm36.567-67.693v-.188c0-19.478-13.736-33.367-37.42-33.367h-25.58v67.5h25.201c23.868.001 37.799-14.468 37.799-33.945zm-37.138 17.361h-7.482V82.841h7.482c10.989 0 18.283 6.265 18.283 17.07v.188c0 10.896-7.294 17.069-18.283 17.069z"/></svg></button></div><span> 或 </span>'
        : "") +
      '<div class="comment-form-guest"><input class="comment-form-input comment-form-name" type="text" name="name" placeholder="名字（必填）" autocomplete="name"><input class="comment-form-input comment-form-email" type="email" name="email" placeholder="邮箱（必填）" autocomplete="email"><input class="comment-form-input comment-form-url" type="url" name="url" placeholder="网址（可选）" autocomplete="url"></div></div>\n' +
      "        </div>\n" +
      "    </div>\n" +
    ...
    _.box = _.dom
      ...
      .replace("comment-form-wrapper", "comment-form-wrapper editing")
      .replace(/加入讨论……/, "");
    _.handle = {
  ...
  iDisqus.prototype.getlist = function () {
    ...
    getAjax(
      _.opts.api +
        "/getcomments.php?ident=" +
        _.opts.identifier +
        (!!_.stat.next ? "&cursor=" + _.stat.next : ""),
      function (resp) {
        var data = JSON.parse(resp);
        if (data.code === 0) {
          ...
          _.dom.querySelector(".comment-avatar-image").dataset.avatar =
            data.forum.avatar;
          if (_.user.logged_in == "false") {
            _.dom.querySelector(".comment-avatar-image").src =
              data.forum.avatar;
          }
          ...
          _.dom.querySelector("#comment-count").innerHTML =
            _.stat.count + " 条评论";
          ...
          if (data.cursor.hasPrev) {
            _.stat.root.forEach(function (item) {
              _.dom
                ...
                .appendChild(_.dom.querySelector("#comment-" + item));
            });
          } else {
  ...
  iDisqus.prototype.load = function (post) {
    ...
    var parentPostDom = _.dom.querySelector(
      '.comment-item[data-id="' + post.parent + '"]'
    );
    ...
    var parentPost = !!post.parent
      ? {
          name:
            '<a class="comment-item-pname" href="#' +
            parentPostDom.id +
            '"><svg class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="200" height="200"><path d="M1.664 902.144s97.92-557.888 596.352-557.888V129.728L1024 515.84l-425.984 360.448V628.8c-270.464 0-455.232 23.872-596.352 273.28"></path></svg>' +
            parentPostDom.dataset.name +
            "</a>",
      ...
      : {
          ...
          insert:
            post.id == "preview" || !!post.isPost ? "afterbegin" : "beforeend",
        };

    var mediaHTML = "";
    if (post.media.length == 1) {
      mediaHTML =
        '<div class="comment-item-images"><a class="comment-item-image" target="_blank" href="' +
        post.media[0] +
        '" style="background-image: url(' +
        post.media[0] +
        ');width: 167px;height: 167px"></a></div>';
    } else if (post.media.length > 1) {
      post.media.forEach(function (item) {
        mediaHTML +=
          '<a class="comment-item-image" target="_blank" href="' +
          item +
          '" style="background-image: url(' +
          item +
          ')"></a>';
    ...
    var html =
      ...
      (post.url ? post.url : "javascript:;") +
      '">' +
      post.name +
      "</a>" +
      (post.isMod
        ? '<span class="comment-item-badge">' + _.opts.badge + "</span>"
        : "") +
      parentPost.name +
    ...
    if (!post.isDeleted) {
      _.dom
        .querySelector(
          '.comment-item[data-id="' + post.id + '"] .comment-item-reply'
        )
        .addEventListener("click", _.handle.show, false);
      _.dom
        .querySelector(
          '.comment-item[data-id="' + post.id + '"] .comment-item-avatar'
        )
        .addEventListener("click", _.handle.jump, false);
      if (!!post.parent) {
        _.dom
          .querySelector(
            '.comment-item[data-id="' + post.id + '"] .comment-item-pname'
          )
          .addEventListener("click", _.handle.jump, false);
      }
    }
    ...
    if (!!post.isPost && !_.stat.editing) {
      ...
      // 删除
      $this.querySelector(".comment-item-delete").addEventListener(
        "click",
        function (e) {
          var postData = {
            id: post.id,
      ...
      // 编辑
      $this.querySelector(".comment-item-edit").addEventListener(
        "click",
        function () {
          _.stat.editing = post;
          _.edit(post);
      ...
      if (showUsers.length > 0) {
        showUsers.forEach(function (item, i) {
          list +=
            ...
            item.username +
            '</div><div class="mention-user-name">' +
            item.name +
            "</div></li>";
        });
        if (!!mentionDom) {
          mentionDom.innerHTML =
            '<ul class="mention-user-list">' + list + "</ul>";
          mentionDom.style.left = coord.left + "px";
          mentionDom.style.top = coord.top + "px";
        } else {
          html =
            '<div class="mention-user" style="left:' +
            coord.left +
            "px;top:" +
            coord.top +
            'px"><ul class="mention-user-list">' +
            list +
            "</ul></div>";
  ...
  iDisqus.prototype.field = function (e) {
    ...
    var selStart = textarea.selectionStart;
    var shortCode =
      selStart == 0 ? item.dataset.code + " " : " " + item.dataset.code + " ";
    textarea.value =
  ...
  iDisqus.prototype.show = function (e) {
    ...
    if ($this.className == "comment-item-reply") {
      ...
      var commentBox = _.box
        .replace(/emoji-input/g, "emoji-input-" + item.dataset.id)
        .replace(/upload-input/g, "upload-input-" + item.dataset.id);
      item
        .querySelector(".comment-item-children")
        .insertAdjacentHTML("beforebegin", commentBox);
      _.user.init();
  ...
  iDisqus.prototype.upload = function (e) {
      ...
      if (xhrUpload.readyState == 4 && xhrUpload.status == 200) {
        ...
        if (data.code == 0) {
          ...
          image.onload = function () {
            $item.innerHTML =
              '<img class="comment-image-object" src="' + imageUrl + '">';
    ...
    xhrUpload.upload.addEventListener(
      ...
      function (e) {
        ...
        var imageItem =
          '<li class="comment-image-item loading" data-image-size="' +
          size +
          '">\n' +
        ...
        form
          ...
          .insertAdjacentHTML("beforeend", imageItem);
  ...
  iDisqus.prototype.post = function (e) {
    var _ = this;
    var item =
      e.currentTarget.closest(".comment-box[data-current-id]") ||
      e.currentTarget.closest(".comment-item") ||
      e.currentTarget.closest(".comment-box");
    ...
    if (!item.dataset.currentId) {
      ...
      if (!!_.opts.emoji_preview) {
        preMessage = preMessage.replace(/:([-+\w]+):/g, function (match) {
          ...
          var emojiImage = !!_.eac[emojiShort]
            ? '<img class="emojione" width="24" height="24" alt="' +
              emojiShort +
              '" title=":' +
              emojiShort +
              ':" src="' +
  ...
  iDisqus.prototype.edit = function (post) {
    var _ = this;
    var commentBox = _.box
      .replace("comment-box", "comment-box comment-box-" + post.id)
      .replace(/emoji-input/g, "emoji-input-" + post.id)
      .replace(/upload-input/g, "upload-input-" + post.id);
    var $this = _.dom.querySelector(
      '.comment-item[data-id="' + post.id + '"] .comment-item-body'
    );
    ...
    item.querySelector(".comment-form-cancel").addEventListener(
      "click",
      function () {
        _.stat.editing = false;
        _.load(post);
  ...
  iDisqus.prototype.create = function () {
    ...
    if (_.opts.autoCreate) {
      _.dom.querySelector(".loading-container").dataset.tip =
        "正在创建 Thread……";
      var postData = {
        url: _.opts.link,
    ...
    _.dom.querySelector("#idisqus").classList.remove("loading");
    _.dom.querySelector("#idisqus").innerHTML =
      '<div class="comment-header"><span class="comment-header-item">创建 Thread</span></div>' +
      '<div class="comment-thread-form">' +
      "<p>由于 Disqus 没有本页面的相关 Thread，故需先创建 Thread</p>" +
      '<div class="comment-form-item"><label class="comment-form-label">url:</label><input class="comment-form-input" id="thread-url" name="url" value="' +
      _.opts.link +
      '" disabled /></div>' +
      '<div class="comment-form-item"><label class="comment-form-label">identifier:</label><input class="comment-form-input" id="thread-identifier" name="identifier" value="' +
      _.opts.identifier +
      '" disabled /></div>' +
      '<div class="comment-form-item"><label class="comment-form-label">title:</label><input class="comment-form-input" id="thread-title" name="title" value="' +
      _.opts.title +
      '" disabled /></div>' +
      '<div class="comment-form-item"><label class="comment-form-label">slug:</label><input class="comment-form-input" id="thread-slug" name="slug" value="' +
      _.opts.slug +
      '" /></div>' +
      '<div class="comment-form-item"><label class="comment-form-label">message:</label><textarea class="comment-form-textarea" id="thread-message" name="message">' +
      _.opts.desc +
      "</textarea></div>" +
      '<button id="thread-submit" class="comment-form-submit">提交</button></div>';
    _.dom
      .querySelector("#thread-submit")
      .addEventListener("click", _.handle.postThread, false);
  };
  ...
  /* Global */ else global["iDisqus"] = global["iDisqus"] || iDisqus;
})(window || this);
